...
function BindItemResult(a) {
    ...
    var returnString = "";
    $.each(queryRaw, function () {
        returnString = returnString + this.type + ":" + this.value + ";";
    });
    $('#ItemLink', parent.document.body).val(returnString);


}

function BindItemResultId(a) {

    $('#ItemLink', parent.document.body).val(a);
...
function OnComplete(a) {
    ...
    if ($("#grid-content").hasClass('mainmargin')) {
        ...
        (a.items,
            function () {
                if (a.items != 0) {
                    b = b + '<div onclick="' + "BindItemResultId('" + this.ItemId + "'); return false;" + '" class="post-1 post type-post status-publish format-standard hentry category-inspiration category-landscapes category-portraits category-typography category-web-design category-weddings tag-image tag-lightbox tag-sample post_float rounded" id="post-1" style="' + Meta(this) + '">' + "<a class=\"ceebox imgcontainer\" title=\"Lightbox Example\" href=\"\"  onclick=\"BindItemResultId('" + this.ItemId + "'); return false;" + '">' + ' <img width="142" onerror="this.onerror=null;this.src=\'../ItemBuckets/images/default.jpg\';" height="100" src="' + this.ImagePath + '?w=142&h=100&db=master " class="attachment-post-thumbnail wp-post-image" ' + '  alt="' + this.Name + '" title="' + this.Name + '" /></a>' + " <h2> " + " <a class=\"ceebox\" title=\"Lightbox Example\" href=\"\" onclick=\"BindItemResultId('" + this.ItemId + "'); return false;" + '">' + this.Name + "  </a></h2> " + ' <div class="post_tags"> ' + '<strong>Template: </strong>' + this.TemplateName + ' <strong>Location: </strong>' + this.Bucket + "<br/><p>" + (this.Content.length > 40 ? (this.Content.substring(0, 40) + "...") : this.Content) + "</p> <strong>Version: </strong>" + this.Version + " <strong>Created:</strong> " + this.Cre + " <strong>By:</strong> " + this.CreBy + "<br />" + " </div>" + " </div>"
                }
            }
        );
        ...
        (a.items,
            function () {
                if (a.items != 0) {
                    if (this.Name != null) {
                        $("#results").append('<li class="BlogPostArea" onclick="' + "BindItemResultId('" + this.ItemId + "'); return false;" + '" style="margin-left:' + InnerItem(this) + '">' + '<div class="BlogPostViews">' + "<a class=\"ceebox imgcontainer\" title=\"Lightbox Example\" href=\"\"  onclick=\"BindItemResultId('" + this.ItemId + "'); return false;" + '">' + ' <img width="80" onerror="this.onerror=null;this.src=\'../ItemBuckets/images/default.jpg\';" height="60" src="' + this.ImagePath + '?w=80&h=60&db=master " class="attachment-post-thumbnail wp-post-image" ' + '  alt="' + this.Name + '" title="' + this.Name + '" /></a>' + "</div>" + '<h5 class="BlogPostHeader">' + '   <a href="#" onclick="' + "BindItemResultId('" + this.ItemId + "'); return false;" + '">' + this.Name + "</a></h5>" + '<div class="BlogPostContent"><strong>Template: </strong>' + this.TemplateName + ' - <strong>Location: </strong>' + this.Bucket + "</div>" + '<div class="BlogPostFooter">' + this.Content + "   <div>" + " <strong>Version: </strong>" + this.Version + '      <strong>Created: </strong>' + this.Cre + "        <strong> by</strong>" + '    ' + this.CreBy + " </div>" + "<div>" + "</div>" + "</li>")
                    }
                    else {
                                        ...
                                        $.each(c, function () {
                                            if (this != "") {
                                                if ((scope.data.indexOf("RecentlyModified") > 0) || (scope.data.indexOf("RecentlyCreated")) > 0 || (scope.data.indexOf("RecentTabs")) > 0) {
                                                    ...
                                                    e = e + "<li><a href=\"#\" onclick=\"BindItemResultId(" + splitMe[1] + "); return false;\" title='Click this to launch a search based on the '" + this + '" class=\"command\" id="' + splitMe[1] + "' style=\"background: url(\'images/pin.png\') no-repeat left center;padding: 0px 18px;\">" + (splitMe[0].length > 20 ? (splitMe[0].substring(0, 20) + "...") : splitMe[0]) + "</a></li>"
