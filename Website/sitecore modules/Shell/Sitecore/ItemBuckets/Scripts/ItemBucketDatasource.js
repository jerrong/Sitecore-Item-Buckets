...
function retrieveFilters() {
    ...
    var a = $("#ui_element");
    var filterArray = ["filetype", "version", "debug", "id", "ref", "custom", "sort", "site", "author", "language", "text", "template", "tag", "start", "end", "recent"];
    $.each(filterArray, function (index, filter) {
    ...
    if ($("#grid-content").hasClass('mainmargin')) {
        ...
        (a.items,
            function () {
                if (a.items != 0) {
                    if (this.Name != null) {
                        $("#results").append('<li class="BlogPostArea" style="margin-left:' + InnerItem(this) + '">' + '<div class="BlogPostViews">' + "<a class=\"ceebox imgcontainer\" title=\"Lightbox Example\" href=\"\"  onclick=\"BindItemResultId('" + this.ItemId + "'); return false;" + '">' + ' <img width="80" onerror="this.onerror=null;this.src=\'../ItemBuckets/images/default.jpg\';" height="60" src="' + this.ImagePath + '?w=80&h=60&db=master " class="attachment-post-thumbnail wp-post-image" ' + '  alt="' + this.Name + '" title="' + this.Name + '" /></a>' + "</div>" + '<h5 class="BlogPostHeader">' + '   <a href="#" onclick="' + "BindItemResultId('" + this.ItemId + "'); return false;" + '">' + this.Name + "</a></h5>" + '<div class="BlogPostContent"><strong>Template: </strong>' + this.TemplateName + ' - <strong>Location: </strong>' + this.Bucket + "</div>" + '<div class="BlogPostFooter">' + this.Content + "   <div>" + " <strong>Version: </strong>" + this.Version + '      <strong>Created: </strong>' + this.Cre + "        <strong> by</strong>" + '    ' + this.CreBy + " </div>" + "<div>" + "</div>" + "</li>")
                    }
                    else {
