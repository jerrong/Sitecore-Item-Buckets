var FieldNameToPassThrough = frameElement.parentElement.previousSibling.outerHTML;
...
if (jQuery.browser.msie) {
    ...
    text = div.textContent.replace(":", "");
}


function scGetFrameValue(value, request) {
    if (request.parameters == "contenteditor:save" || request.parameters == "item:save") {
        var b = $('#EditorFrames iframe[sc_value]', window.parent.document).attr('sc_value');
        //populate the new id list
        parent.scForm.modified = true;
        $('#EditorFrames iframe[sc_value]', window.parent.document).attr('sc_value', b);
    }
}
...
jQuery.ajax({
    type: "POST",
    url: "/sitecore%20modules/Shell/Sitecore/ItemBuckets/ItemBucket.asmx/GetFieldValues",
    data: "{'FieldName' : '" + text + "'}",
    ...
    success: function(a) {
        ...
        var searchList = b.toString().split('|');
        var outPut = "";
        outPut = outPut + "<ul>";
        for (var i = 0; i < searchList.length; i++) {
            if (searchList[i] != "") {
                outPut = outPut + '<li class="token-input-token-facebook"><span style="background: url(\'images/id.gif\') no-repeat center center;padding: 0px 11px;"></span><p class="id">' + searchList[i] + '</p><p class="id">' + searchList[i + 1] + '</p><span class="token-input-delete-token-facebook removeid">×</span></li>';
            }
            i++;
        }
        outPut = outPut + "</ul>";

        $("#savedIds").html(outPut);
    }
});

$(".removeid").live("click", function() {

    var returnId = "";
    if ($(this)[0].previousElementSibling == undefined) {
        returnId = $(this)[0].parentElement.innerText;
        returnId = returnId.substring(0, returnId.length - 1)
    } else {
        returnId = $(this)[0].previousElementSibling.innerHTML;
    }

    jQuery.ajax({
        type: "POST",
        url: "/sitecore%20modules/Shell/Sitecore/ItemBuckets/ItemBucket.asmx/RemoveFieldValues",
...
function saveFieldValue(id) {
    ...
    jQuery.ajax({
        type: "POST",
        url: "/sitecore%20modules/Shell/Sitecore/ItemBuckets/ItemBucket.asmx/SaveFieldValues",
    ...
    if ($("#grid-content").hasClass('mainmargin')) {
        ...
        (a.items,
            function () {
                if (a.items != 0) {
                    if (this.Name != null) {
                        $("#results").append('<li class="BlogPostArea" style="margin-left:' + InnerItem(this) + '">' + '<div class="BlogPostViews">' + "<a class=\"ceebox imgcontainer\" title=\"Lightbox Example\" href=\"\"  onclick=\"saveFieldValue('" + this.ItemId + "'); return false;" + '">' + ' <img width="80" onerror="this.onerror=null;this.src=\'../ItemBuckets/images/default.jpg\';" height="60" src="' + this.ImagePath + '?w=80&h=60&db=master " class="attachment-post-thumbnail wp-post-image" ' + '  alt="' + this.Name + '" title="' + this.Name + '" /></a>' + "</div>" + '<h5 class="BlogPostHeader">' + '   <a href="#" onclick="' + "saveFieldValue('" + this.ItemId + "'); return false;" + '">' + this.Name + "</a></h5>" + '<div class="BlogPostContent"><strong>Template: </strong>' + this.TemplateName + ' - <strong>Location: </strong>' + this.Bucket + "</div>" + '<div class="BlogPostFooter">' + this.Content + "   <div>" + " <strong>Version: </strong>" + this.Version + '      <strong>Created: </strong>' + this.Cre + "        <strong> by</strong>" + '    ' + this.CreBy + " </div>" + "<div>" + "</div>" + "</li>")
                    }
                    else {
